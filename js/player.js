...
if (navigator.userAgent.match(/(iPhone|iPod|iPad|Android|ios|Nokia|Black Berry|MIDP|Phone)/i) && mobiles === "no") {
	...
	if (myhkLoad && window.location.href.indexOf("music.m0x.cn") < 0) {
		 ...
		 * 本地调试使用
		 */
		var webURL = 'karune.tk';
		var keyId = $("#xplayer").attr("key");
		console.log("欢迎使用：" + webURL);
		localStorage.setItem("myhkLoad", "true");
		$("head").append("<link rel=\"stylesheet\" type=\"text/css\" href=\""+webURL+"/js/player.css\">");
...
"<div id=\"myhkLrc\"></div>");
		if (top.location != self.location) {
			...
			$songFrom.html("<a style=\"color:#f00\">玫曦</a>");
				...
				creat: {
					album: function() {
						if (typeof myhkid === "undefined") {
							...
							$(".musicheader", $albumList).html("<a href=\"" + webURL + "\" title=\"欢迎使用玫曦播放器</a>")
						};
			...
			$.ajax({
				...
				success: function() {
					if (typeof myhkid !== "undefined") {
						$(".myhkgeci", $player).html("<a style=\"float: right\" href=\"" + webURL + "\" title=\"点击免费申请玫曦播放器\" target=\"_blank\"><i class=\"fa fa-share-alt\"></i> 免费申请</a>");
